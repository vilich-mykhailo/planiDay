...
export default function LoginOwner() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2️⃣ Завантаження студій власника
const studios = await api("/owner", {
  token: data.token,
      ...
      if (!Array.isArray(studios) || studios.length === 0) {
        ...
        throw new Error("Не знайдено студій для цього власника");
      }

      const firstStudioId = studios[0].id;
      localStorage.setItem("studioId", firstStudioId);

      // 3️⃣ Повідомляємо всі компоненти про зміну авторизації
      window.dispatchEvent(new Event("auth-changed"));

      // 4️⃣ Перехід на Dashboard
      navigate("/dashboard");
    } catch (err) {
      ...
      <div className="w-full max-w-6xl px-4 py-10 sm:py-14">
        <div className="mx-auto max-w-md">
          ...
          <div className="text-center">
            <h1 className="mt-14 text-xl font-black tracking-tight text-stone-800 sm:text-3xl md:text-4xl">
          ...
          {/* Card */}
          <div className="mt-5 overflow-hidden rounded-2xl border border-stone-200/60 bg-white shadow-sm sm:mt-8 sm:rounded-3xl">
...
<div className="flex justify-between text-xs sm:text-sm">
  ...
  >
    Забули пароль?
                ...
                <button
                  disabled={loading}
                  className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 text-white shadow-lg transition-all hover:from-emerald-700 hover:to-emerald-800 disabled:opacity-60 sm:h-auto sm:rounded-2xl sm:px-5 sm:py-3.5"
                >
                  {loading ? "Вхід..." : <>
              ...
              <div className="mt-5 border-t border-stone-100 pt-4 text-center sm:mt-6 sm:pt-5">
                <p className="text-xs text-stone-600 sm:text-sm">
                  Немає салону?{" "}
                  <Link to="/register-owner" className="font-bold text-stone-800 hover:text-emerald-700 hover:underline">
                    Створити салон
                  </Link>
                </p>
              </div>
