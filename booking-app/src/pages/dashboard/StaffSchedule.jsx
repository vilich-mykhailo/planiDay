...
function Modal({ open, title, children, onClose, footer }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black/60 flex items-center justify-center p-4" onMouseDown={onClose}>
      <div
        ...
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between gap-3">
          ...
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <path d="M6 6l12 12M18 6l-12 12" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" />
            </svg>
          </button>
        </div>
...
const [activeEmployeeId, setActiveEmployeeId] = useState(
  ...
  function updateEmployee(empId, patch) {
    setData((prev) => ({
      ...prev,
      employees: prev.employees.map((e) => (e.id === empId ? { ...e, ...patch } : e)),
    }));
  }
    ...
    <div className="mx-auto max-w-7xl min-h-[100dvh] pb-20">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-gray-600">
              Додавай працівників, плануй зміни по днях та отримуй розрахунок зарплат за місяць.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <aside className="lg:col-span-4 space-y-4">
          <section className="rounded-2xl border border-gray-200 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.06)] overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <div className="flex items-center justify-between gap-3">
                <h2 className="text-base font-extrabold text-gray-900">Працівники</h2>
            ...
            <div className="p-3">
              {!data.employees.length ? (
                <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">
                  Додай першого працівника — після цього зможеш заповнювати графік та рахувати зарплату.
                </div>
              ) : (
                <div className="space-y-2">
                  {data.employees.map((emp) => {
                    const sum = monthSummary.get(emp.id) || { minutes: 0, salary: 0 };
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <p className={["text-sm font-extrabold truncate", emp.id === activeEmployeeId ? "text-white" : "text-gray-900"].join(" ")}>
                              {emp.name}
                            </p>
                            <p className={["mt-0.5 text-xs truncate", emp.id === activeEmployeeId ? "text-white/80" : "text-gray-600"].join(" ")}>
                              {emp.role || "Посада не вказана"} • ставка: {formatMoney(Number(emp.rate) || 0)}/год
                            </p>
                          </div>

                          <div className="shrink-0 text-right">
                            <p className={["text-xs font-bold", emp.id === activeEmployeeId ? "text-white/90" : "text-gray-700"].join(" ")}>
                              {hours.toFixed(1)} год
                            </p>
                            <p className={["text-sm font-extrabold", emp.id === activeEmployeeId ? "text-white" : "text-gray-900"].join(" ")}>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.06)]">
            <div className="px-5 py-4 border-b border-gray-100">
              <h3 className="text-base font-extrabold text-gray-900">Налаштування</h3>
              <p className="mt-1 text-sm text-gray-600">Ставка впливає на розрахунок зарплати.</p>
            </div>
            ...
            <div className="px-5 py-5">
              {!activeEmployee ? (
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        onChange={(e) => updateEmployee(activeEmployee.id, { name: e.target.value })}
                    ...
                    <div className="space-y-1.5">
                      <label className="text-sm font-semibold text-gray-900">Посада</label>
                      <input
                        value={activeEmployee.role || ""}
                        onChange={(e) => updateEmployee(activeEmployee.id, { role: e.target.value })}
                        className="w-full rounded-xl border border-gray-200 bg-white p-3 text-sm font-medium text-gray-900 outline-none transition focus:border-black focus:ring-2 focus:ring-black/40"
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div className="space-y-1.5">
                      <label className="text-sm font-semibold text-gray-900">Ставка (за годину)</label>
                      <input
                        inputMode="decimal"
                        ...
                        onChange={(e) => {
                          ...
                          const num = Number(v);
                          updateEmployee(activeEmployee.id, { rate: Number.isFinite(num) ? num : 0 });
                        }}
                        className="w-full rounded-xl border border-gray-200 bg-white p-3 text-sm font-medium text-gray-900 outline-none transition focus:border-black focus:ring-2 focus:ring-black/40"
                        placeholder="Напр. 150"
                      />
                      <p className="text-xs text-gray-500">Підказка: можна вводити “150.5”.</p>
                    </div>

                    <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                      {(() => {
                        const sum = monthSummary.get(activeEmployee.id) || { minutes: 0, salary: 0 };
                        const hours = sum.minutes / 60;
                        return (
                          <>
                            <p className="text-xs font-semibold text-gray-500">Підсумок за місяць</p>
        ...
        <main className="lg:col-span-8 space-y-4">
          <section className="rounded-2xl border border-gray-200 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.06)] overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <h2 className="text-base font-extrabold text-gray-900 truncate">Розклад на місяць</h2>
                  <p className="mt-1 text-sm text-gray-600">
                    Натисни на день, щоб задати час зміни. Порожня зміна = вихідний.
                  </p>
                </div>
              ...
              {!activeEmployee ? (
                ...
                <>
                  ...
                  <div className="mt-3 grid gap-2">
                    {datesGrid.map((week, wi) => (
                      <div key={wi} className="grid grid-cols-7 gap-2">
                        {week.map((d, di) => {
                          if (!d) {
                            return <div key={di} className="h-[84px] rounded-2xl bg-gray-50 border border-gray-100" />;
                          }

                          const iso = toISODate(d);
                          const s = data.shifts[shiftKey(activeEmployee.id, iso)];
                          const minutes = calcShiftMinutes(s?.start, s?.end);
                          ...
                          return (
                            <button
                              ...
                              className={[
                                ...
                                s ? "border-black bg-black text-white" : "border-gray-200 bg-white hover:bg-gray-50",
                                isWeekend && !s ? "bg-gray-50" : "",
                              ].join(" ")}
                              title="Редагувати зміну"
                            >
                              <div className="flex items-start justify-between gap-2">
                                <div className="min-w-0">
                                  <p className={["text-xs font-bold", s ? "text-white/80" : "text-gray-500"].join(" ")}>
                                    {d.getDate()}
                                    {isToday && (
                                      <span className={["ml-2 rounded-full px-2 py-0.5 text-[10px] font-extrabold", s ? "bg-white/15 text-white" : "bg-black text-white"].join(" ")}>
                                        Сьогодні
                                      </span>
                                    )}
                                  </p>
                                  <p className={["mt-1 text-sm font-extrabold truncate", s ? "text-white" : "text-gray-900"].join(" ")}>
                                    {s ? `${s.start} – ${s.end}` : "Вихідний"}
                                  </p>
                                </div>

                                {s && (
                                  <div className="text-right">
                                    <p className="text-xs font-bold text-white/80">{hours.toFixed(1)} год</p>
                                    <p className="text-sm font-extrabold text-white">
                                      {formatMoney(hours * (Number(activeEmployee.rate) || 0))}
                                    </p>
                                  </div>
                                )}
                              </div>
                              ...
                              {!s && (
                                <span className="absolute right-3 bottom-3 text-xs font-bold text-gray-400">
                                  + зміна
                                </span>
                              )}
                            </button>
                  ...
                  <div className="mt-5 rounded-2xl border border-gray-200 bg-gray-50 p-4">
                    <p className="text-xs font-semibold text-gray-500">Порада</p>
                    <p className="mt-1 text-sm text-gray-700">
                      Якщо вказати однаковий час для всіх робочих днів — можна швидко “наклацати” календар.
                      Пізніше додамо шаблони (Пн–Пт 09:00–18:00) одним кліком.
                    </p>
                  </div>
                </>
              )}
          ...
          <section className="rounded-2xl border border-gray-200 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.06)] overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <h3 className="text-base font-extrabold text-gray-900">Зарплата за місяць</h3>
              <p className="mt-1 text-sm text-gray-600">Підсумок по кожному працівнику.</p>
            </div>
                  ...
                  <table className="w-full text-left">
                    ...
                    <tbody>
                      {data.employees.map((emp) => {
                        ...
                        return (
                          <tr key={emp.id} className={["border-t", isActive ? "bg-gray-50" : ""].join(" ")}>
                            <td className="py-3 pr-3">
                              <div className="min-w-0">
                                <p className="text-sm font-extrabold text-gray-900 truncate">{emp.name}</p>
                                <p className="mt-0.5 text-xs text-gray-500 truncate">{emp.role || "Посада не вказана"}</p>
                              </div>
                            </td>
                            <td className="py-3 pr-3 text-sm font-bold text-gray-900">{formatMoney(Number(emp.rate) || 0)}/год</td>
                            <td className="py-3 pr-3 text-sm font-bold text-gray-900">{hours.toFixed(1)}</td>
                            <td className="py-3 pr-3 text-sm font-extrabold text-gray-900">{formatMoney(sum.salary)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
      ...
      >
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-semibold text-gray-900">Ставка / год</label>
            <input
              inputMode="decimal"
              value={newEmp.rate}
              onChange={(e) => setNewEmp((p) => ({ ...p, rate: e.target.value }))}
              className="w-full rounded-xl border border-gray-200 bg-white p-3 text-sm font-medium text-gray-900 outline-none transition focus:border-black focus:ring-2 focus:ring-black/40"
              placeholder="150"
            />
          </div>
          ...
          <div className="sm:col-span-2 rounded-2xl border border-gray-200 bg-gray-50 p-4">
            ...
            <p className="mt-1 text-sm text-gray-700">
              Далі обереш працівника → натискатимеш на дні календаря → задаватимеш час зміни.
              Зарплата рахується автоматично: <span className="font-semibold">години × ставка</span>.
            </p>
          </div>
        </div>
      </Modal>
      ...
      <Modal
        open={edit.open}
        title={activeEmployee ? `Зміна: ${activeEmployee.name} • ${edit.iso}` : `Зміна • ${edit.iso}`}
      ...
      >
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            <label className="text-sm font-semibold text-gray-900">Початок</label>
            <input
              type="time"
              value={edit.start}
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-semibold text-gray-900">Кінець</label>
            <input
              type="time"
              value={edit.end}
          ...
          <div className="sm:col-span-2 rounded-2xl border border-gray-200 bg-gray-50 p-4">
            {(() => {
              ...
              return (
                <>
                  <p className="text-xs font-semibold text-gray-500">Підсумок зміни</p>
                  <p className="mt-1 text-sm font-extrabold text-gray-900">
                    {hours.toFixed(2)} год • {formatMoney(hours * rate)}
                  </p>
                  <p className="mt-2 text-xs text-gray-600">
                    Якщо час некоректний або 0 год — зміна буде очищена (вихідний).
                  </p>
                </>
              );
            })()}
